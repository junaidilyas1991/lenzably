(function($) {
  ...
  function layout(container, options, items) {
    ...
    var containerWidth =
      Math.floor(containerBoundingRect.right - containerBoundingRect.left) -
    ...
    for (var index = 0; index < itemsSize; ++index) {
      if (items[index].classList) {
        ...
        // IE <10
        items[index].className = items[index].className.replace(
          new RegExp(
            "(^|\\b)" +
              options.firstItemClass +
              "|" +
              options.lastItemClass +
              "|" +
      ...
      if (index === itemsSize - 1) {
        for (
          ...
          rowElems[rowElemIndex].style.cssText =
            "width: " +
            itemAttrs[index + parseInt(rowElemIndex) - rowElems.length + 1]
              .width +
            "px;" +
            "height: " +
            itemAttrs[index + parseInt(rowElemIndex) - rowElems.length + 1]
            ...
            "px;" +
            "margin-right:" +
            (rowElemIndex < rowElems.length - 1 ? options.minMargin + "px" : 0);
        }
      }

      // check whether width of row is too high
      if (
        rowWidth + options.maxMargin * (rowElems.length - 1) >
        containerWidth
      ) {
        var diff =
          rowWidth + options.maxMargin * (rowElems.length - 1) - containerWidth;
        ...
        for (
          ...
          rowElem.style.cssText =
            ...
            "px;" +
            "margin-right: " +
            (rowElemIndex < rowElems.length - 1 ? rowMargin : 0) +
            "px";
          if (rowElemIndex === 0) {
